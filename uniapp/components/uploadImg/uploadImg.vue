<template>
  <view class="imglistbx">
		<block v-if="type=='card'">
			<view class="identification_card" @click="uploadImg('')">
					...
					<block v-if="imgList.length>0">
						<view class="itemImg flex_layout">
							<view class="uploadimg_list" v-for="(item,index) in imgList" :key='index'>
								...
								<view class="deleteIcon" v-if="maxCount>1" @click="deleteImg(index)">X</view>
							</view>
						</view>
					</block>
...
<style scoped>
  ...
  .clear {
	...
	.uploadImg_btn{width: 88rpx; height: 88rpx; background: #f3f2f7; border-radius: 5px; margin-left: 10rpx; display: flex; align-items: center; justify-content: center;}
	.uploadImg_btn image{width: 42rpx; height: 42rpx; display: block; margin: 0 auto;}
	.uploadImg_btn text{display: block; color: #999; font-size: 24rpx; margin-top: 10rpx;text-align: center}
	.uploadimg_list{width: 88rpx; height: 88rpx;  position: relative;margin: 0 10rpx;}
	.uploadimg_list image{width: 100%; height: 100%;border-radius: 5px;}
