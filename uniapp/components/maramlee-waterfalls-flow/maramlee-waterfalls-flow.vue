<template>
  <view class="waterfalls-box" :style="{ height: height + 'px' }" v-if="list.length>0">
    <!--  #ifdef  MP-WEIXIN -->
    <view
    ...
    >
      ...
      />
      <slot @click="$emit('detail-lick', item)" name="slot{{index}}" />
    </view>
    <!--  #endif -->
    ...
    >
      <image
        ...
        @load="imageLoadHandle(index)"
        @error="imageLoadHandle(index)"
        @click.stop="$emit('image-click', item,index)"
      />
      <slot v-bind="item" />
    </view>
    <!--  #endif -->
