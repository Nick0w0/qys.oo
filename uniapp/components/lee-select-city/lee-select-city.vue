		...
		<view class="lee-select-display" v-if="currentSelected.length">
			<view class="cu-list flex-sub">
				<view class="cu-item">
					...
					<view>
						<button class="cu-btn bg-purple shadow-blur" @tap="searchCity" data-target="gridModal"><text class="cuIcon-search margin-right-xs"></text>搜索</button> 
					</view>
				</view>
			</view>
				...
				<scroll-view class="swiper-item" scroll-y>
					<view class="swiper-content">
						...
						<block v-if="hotCities.length">
							...
							<view class="gird">
								<!-- <view class="gird-item"><view @click="hotCitySelectHandler(-1)">全部</view></view> -->
	...
	export default {
		...
		methods: {
			...
			hotCitySelectHandler(item) {
				console.log("111: ",111);
				if(item==-1){
					this.$db.del('cityName');
					let pages = getCurrentPages();  //获取所有页面栈实例列表
					let nowPage = pages[ pages.length - 1];  //当前页页面实例
					let prevPage = pages[ pages.length - 2 ];  //上一页页面实例
					prevPage.$vm.cityName = '全国';   //修改上一页data里面的searchVal参数值为1211
