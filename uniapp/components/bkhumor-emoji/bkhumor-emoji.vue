<template>
	<view class="emotion-box">
		...
		<swiper-item class="emotion-box-line">
			<view class="expression_view" v-for="(line, i) in list1" v-bind:key="i">
		...
		<swiper-item class="emotion-box-line">
			<view class="expression_view" v-for="(line, i) in list2" v-bind:key="i">
				<view v-for="(im, ix) in line" v-bind:key="ix" class="swiper-item emotion-item" :style="{width:img_width+'px'}" @click="clickHandler(im)">
					<rich-text :nodes="im.toString().replace(/^([1-9]\d?|97)$/g, emotion(im))"></rich-text> 
				</view>
			</view>
		...
		<swiper-item class="emotion-box-line">
			<view class="expression_view" v-for="(line, i) in list3" v-bind:key="i">
				<view v-for="(im, ix) in line" v-bind:key="ix" class="swiper-item emotion-item" :style="{width:img_width+'px'}" @click="clickHandler(im)">
					<rich-text :nodes="im.toString().replace(/^([1-9]\d?|97)$/g, emotion(im))"></rich-text> 
				</view>
			</view>
		...
		<swiper-item class="emotion-box-line">
			<view class="expression_view" v-for="(line, i) in list4" v-bind:key="i">
				<view v-for="(im, ix) in line" v-bind:key="ix" class="swiper-item emotion-item" :style="{width:img_width+'px'}" @click="clickHandler(im)">
					<rich-text :nodes="im.toString().replace(/^([1-9]\d?|97)$/g, emotion(im))"></rich-text> 
				</view>
			</view>
		...
		<swiper-item class="emotion-box-line">
			<view class="expression_view" v-for="(line, i) in list5" v-bind:key="i">
				<view v-for="(im, ix) in line" v-bind:key="ix" class="swiper-item emotion-item" :style="{width:img_width+'px'}" @click="clickHandler(im)">
					<rich-text :nodes="im.toString().replace(/^([1-9]\d?|97)$/g, emotion(im))"></rich-text> 
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			emotion(res) {
				...
				//  请将此处地址更换为自己服务器的地址
				return `<img src="${this.cndUrl}/uploads/uni-emoji/${index}.gif" align="middle">`
			}
		}
	}
</script>
