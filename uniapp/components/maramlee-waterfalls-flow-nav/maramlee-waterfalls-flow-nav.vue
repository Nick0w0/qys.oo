  ...
  <view>
    <!-- 
	 ...
	 <view class="cu-bar bg-gradual-purple search">
	 	<view class="cu-avatar round" :style="{'background-image':'url('+baseLogo+')'}"  @tap="user"></view>
        ...
        >
          <view v-if="weixin_type === 'default'" class="cnt">
			...
			<view class="user flex_layout">
				<view><image src="https://ossweb-img.qq.com/images/lol/web201310/skin/big10006.jpg"></image>一栗沙子</view>
				<view>
...
export default {
  ...
  created() {
	  console.log("this.navIndex111: ",this.navIndex);
    this.moreNavIndex = this.navIndex;
  ...
  methods: {
    ...
    navClickHandle(key, index) {
      if (!this.isLoading) { 
		...
		console.log(this.moreNavIndex);
        this.$emit("nav-click", key,index);
        this.$nextTick(() => {
